...
**Важно:** после `"url": "..."` обязательна **запятая**, если ниже идёт ещё поле (например `"cert"`). Шаблон: `servers.json.example` в репозитории.
...
├── servers.json.example         # Пример синтаксиса (в т.ч. запятая после url)
